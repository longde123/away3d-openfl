...
/**
    ...
    public function setRenderState(stage3DProxy:Stage3DProxy, renderable:IRenderable, vertexConstantOffset:Int, vertexStreamOffset:Int, camera:Camera3D):Void {
        ...
        var state:ParticleStateBase;
        if (!subMesh) throw (new Error("Must be subMesh"));
        if (!subMesh.animationSubGeometry) _particleAnimationSet.generateAnimationSubGeometries(subMesh.parentMesh);
...
//process animator subgeometries
        if (!subMesh.animatorSubGeometry && _animatorParticleStates.length) generateAnimatorSubGeometry(subMesh);
