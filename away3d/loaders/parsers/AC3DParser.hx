...
class AC3DParser extends ParserBase {
    ...
    private var _trunk:Array<Dynamic>;
    private var _containersList:Array<Dynamic>;
    private var _tmpcontainerpos:Vector3D;
    private var _tmpos:Vector3D;
    private var _kidsCount:Int;
...
/**
    ...
    static public function supportsData(data:Dynamic):Bool {
        ...
        else str = ((Std.is(data, String))) ? Std.string(data).substr(0, 4) : null;
...
/**
    ...
    override private function resolveDependency(resourceDependency:ResourceDependency):Void {
        ...
        if (mesh && asset) {
            if (materialMode < 2) cast((mesh.material), TextureMaterial).texture = asset
            else cast((mesh.material), TextureMultiPassMaterial).texture = asset;
        }
    }
...
//var m:Mesh;
        ...
        while (_charIndex < _stringLen && hasTime()) {
            ...
            if (_charIndex == -1) _charIndex = _stringLen;
            ...
            if (_charIndex != _stringLen) _oldIndex = _charIndex + 1;
            var _sw0_ = (_trunk[0]);
            switch(_sw0_) {
                case "MATERIAL":
                    generateMaterial(line);
                case "numsurf", "crease", "texrep", "refs lines of", "url", "data", "numvert lines of", "SURF":
                    switch(_sw0_) {
                        case "numsurf":
//integer
                    }
...
//0x30
                case "kids":
//howmany children in the upcomming object. Probably need it later on, to couple with container/group generation
                ...
                case "OBJECT":
                    ...
                    if (_trunk[1] == "world") _lastType = "world"
...
//in case of groups, numvert might not be there
                ...
                case "name":
                    ...
                    if (_lastType == "poly") _activeMesh.name = nameid
                    else _activeContainer.name = nameid;
                case "numvert":
                ...
                case "refs":
                    ...
                    else if (refscount < 3 || refscount > 4) continue;
                    else _isQuad = false;
                    _parsesV = false;
                case "mat":
                    if (!_activeMesh.material) _activeMesh.material = _materialList[parseInt(_trunk[1])];
                case "texture":
                    if (materialMode < 2) _activeMesh.material = new TextureMaterial(DefaultMaterialManager.getDefaultTexture())
                    else _activeMesh.material = new TextureMultiPassMaterial(DefaultMaterialManager.getDefaultTexture());
...
//_activeMesh.transform = matrix;
                default:
                    if (_trunk[0] == "") break;
                    if (_parsesV) _vertices.push(new Vertex(-(parseFloat(_trunk[0])), parseFloat(_trunk[1]), parseFloat(_trunk[2])))
                    else {
                        if (_isQuad) {
                            _quadCount++;
                            if (_quadCount == 4) {
                            ...
                            else _uvs.push(parseInt(_trunk[0]), new UV(parseFloat(_trunk[1]), 1 - parseFloat(_trunk[2])));
                        }
...
/*
    ...
    private function parseMaterialLine(materialString:String):MaterialBase {
        ...
        while (i < trunk.length) {
            ...
            switch(_sw1_) {
                case "rgb":
                    var r:Int = (parseFloat(trunk[i + 1]) * 255);
                    var g:Int = (parseFloat(trunk[i + 2]) * 255);
                    var b:Int = (parseFloat(trunk[i + 3]) * 255);
                    i += 3;
                    color = r << 16 | g << 8 | b;
                case "amb":
