...
//private var _charIndex:uint;
    ...
    private var _colorTable:Array<Dynamic>;
/**
...
/**
    ...
    static public function supportsData(data:Dynamic):Bool {
        ...
        if (!str) return false;
        if (str.indexOf("ENDSEC") != -1 && str.indexOf("EOF") != -1) return true;
        return false;
    }
...
//ignoring visibility tag
                                if (Math.isNaN(lineVal) && tag == "8" && _vSet == 0) _meshName = line;
                            default:
                                if (Math.isNaN(lineVal) && tag == "8" && _vSet == 0) _meshName = line;
                        }
                    }
                        ...
                        switch(tag) {
                            case "8":
                                if (Math.isNaN(lineVal)) _meshName = line;
                            case "10":
                                _v0.x = lineVal;
                                _vSet++;
                            case "20":
...
// 128, is the closing tag for a face.
                                if (lineVal != 128 && _vSet == 3) _polyLines.push(_v0.clone());
                                _vSet = 0;
                            case "71", "72":
...
//in case of negative, invisible edges (line draw for faces not supported anyway)
                                _polyLinesIndices.push(int(Math.abs(lineVal)) - 1, -1);
//pushing already 4th component to make sure all is quad
                            case "74":
...
// we try display some data from this pourly defined dxf file. Purely to give some visual clues. Chances for holes very high.
            ...
            _v1 = _polyLines[1];
            _v2 = _polyLines[2];
            if (_polyLines.length >= 4) _v3 = _polyLines[3]
            else _v3 = _v2;
            finalizeFace();
        }
...
//indices were set in the vertex tags so we expect 4 indices per face (we forced push a negative index to make sure there are 4)
            if (_polyLinesIndices.length % 4 == 0) {
                ...
                while (i < _polyLinesIndices.length) {
                    ...
                    if (_polyLinesIndices[i + 3] > -1) _v3 = _polyLines[_polyLinesIndices[i + 3]]
                    else _v3 = _v2;
                    finalizeFace();
                    i += 4;
                }
...
// if v3 values are not equal to v2, it's a quad.
    ...
    private function buildMesh():Mesh {
        ...
        if (!_itemColor || Math.isNaN(_itemColor)) {
        ...
        else color = _itemColor;
        if (materialMode < 2) material = new ColorMaterial(color)
        else material = new ColorMultiPassMaterial(color);
    ...
    private function finalizeLine():Void {
        ...
        var lineColor:Int = ((!_itemColor || Math.isNaN(_itemColor))) ? 0xCCCCCC : _itemColor;
