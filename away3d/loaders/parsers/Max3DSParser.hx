...
class Max3DSParser extends ParserBase {
    ...
    private var _unfinalized_objects:Dynamic;
...
/**
    ...
    static public function supportsData(data:Dynamic):Bool {
        ...
        if (ba) {
            ba.position = 0;
            if (ba.readShort() == 0x4d4d) return true;
        }
        return false;
    }
...
// the last one in that object, finalize the current object.
            if (_cur_mat && _byteData.position >= _cur_mat_end) finalizeCurrentMaterial()
...
// Transform
                    ...
                    case 0xB002:
// Object animation (including pivot)
...
// the pipeline.
        if (_byteData.bytesAvailable || _cur_obj || _cur_mat) return MORE_TO_PARSE
        else {
            var name:String;
// Finalize any remaining objects before ending.
            for (name in Reflect.fields(_unfinalized_objects)) {
                var obj:ObjectContainer3D;
                obj = constructObject(_unfinalized_objects[name]);
                if (obj) finalizeAsset(obj, name);
            }
...
// Specular color
                ...
                case 0xA081:
// Two-sided, existence indicates "true"
                    mat.twoSided = true;
                case 0xA200:
// Main (color) texture
...
// Skip "face info", irrelevant in Away3D
    ...
    private function parseSmoothingGroups():Void {
        var len:Int = _cur_obj.indices.length / 3;
        var i:Int = 0;
        while (i < len) {
    ...
    private function parseFaceMaterialList():Void {
        ...
        i = 0;
        while (i < faces.length)faces[i++] = _byteData.readUnsignedShort();
...
// TODO: Implement containers in 3DS parser.
        if (name != "$$$DUMMY" && _unfinalized_objects.hasOwnProperty(name)) {
            ...
            if (obj) finalizeAsset(obj, vo.name);
            delete;
    ...
    private function constructObject(obj:ObjectVO, pivot:Vector3D = null):ObjectContainer3D {
        if (obj.type == AssetType.MESH) {
            ...
            if (!obj.indices || obj.indices.length == 0) return null;
...
// vertex is unique if no smoothGroup found
    ...
    private function finalizeCurrentMaterial():Void {
        var mat:MaterialBase;
        if (materialMode < 2) {
            if (_cur_mat.colorMap) mat = new TextureMaterial(_cur_mat.colorMap.texture || DefaultMaterialManager.getDefaultTexture())
            else mat = new ColorMaterial(_cur_mat.diffuseColor);
            ...
            if (_cur_mat.colorMap) mat = new TextureMultiPassMaterial(_cur_mat.colorMap.texture || DefaultMaterialManager.getDefaultTexture())
            else mat = new ColorMultiPassMaterial(_cur_mat.diffuseColor);
    ...
    private function readNulTermString():String {
        ...
        while ((chr = _byteData.readUnsignedByte()) > 0)str += String.fromCharCode(chr);
        return str;
    }
...
// X axis
        data[0] = _byteData.readFloat();
// X
        data[2] = _byteData.readFloat();
// Z
        data[1] = _byteData.readFloat();
// Y
        data[3] = 0;
// Z axis
        data[8] = _byteData.readFloat();
// X
        data[10] = _byteData.readFloat();
// Z
        data[9] = _byteData.readFloat();
// Y
        data[11] = 0;
// Y Axis
        data[4] = _byteData.readFloat();
// X
        data[6] = _byteData.readFloat();
// Z
        data[5] = _byteData.readFloat();
// Y
        data[7] = 0;
// Translation
        data[12] = _byteData.readFloat();
// X
        data[14] = _byteData.readFloat();
// Z
        data[13] = _byteData.readFloat();
// Y
        data[15] = 1;
        return data;
    }

    private function readColor():Int {
        var cid:Int;
        var len:Int;
        var r:Int;
        var g:Int;
