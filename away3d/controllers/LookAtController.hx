...
/**
    ...
    public function new(targetObject:Entity = null, lookAtObject:ObjectContainer3D = null) {
        ...
        super(targetObject);
        if (lookAtObject) this.lookAtObject = lookAtObject
...
/**
    ...
    public function set_lookAtObject(val:ObjectContainer3D):ObjectContainer3D {
        ...
        if (_lookAtObject == val) return;
        if (_lookAtObject) _lookAtObject.removeEventListener(Object3DEvent.SCENETRANSFORM_CHANGED, onLookAtObjectChanged);
        _lookAtObject = val;
        if (_lookAtObject) _lookAtObject.addEventListener(Object3DEvent.SCENETRANSFORM_CHANGED, onLookAtObjectChanged);
        notifyUpdate();
        return val;
    }
